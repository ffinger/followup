...
#' @param date_analysis the date on which to compute the probability of new
#'   symptoms
#' @param exposure a vector containing one or several possible dates of
#'   exposure. They are equally weighted by default.
#' @param exposure_weight a vector containing weights applied to different exposure dates.
#'   same length as exposure. Defaults to rep(1, length(exposure)).
#' @return a probability
#'
#' @export
p_new_onset <- function(incubation_period, date_analysis, exposure, exposure_weight = NULL) {
  ...
  if (is.null(exposure_weight)) {
    exposure_weight <- rep(1, length(exposure))
  }

  if (length(exposure_weight) != length(exposure)) {
    stop("exposure_weight must have the same length as exposure.")
  }

  idx_weight <- 1:length(exposure_weight)

  # normalize exposure weight
  exposure_weight <- exposure_weight/sum(exposure_weight)
  ...
  idx_weight <- idx_weight[idx <= length(incubation_period)] #remove index larger than maximal incubation period
  ...
  idx_weight <- idx_weight[idx > 0] #remove index for exposure dates after analysis dates
  ...
  if (length(idx) == 0) {
    ...
    p <- sum(exposure_weight[idx_weight] * incubation_period[idx])
  }
